...
struct PFS0FileEntry {
    ...
    let name: String
    // Data is now an optional DataProvider, not the data itself.
...
struct PFS0Partition: PrettyPrintable, JSONSerializable {
    ...
    func extractFiles(to extractor: FileExtractor) throws {
        ...
        for entry in files {
            ...
            // Read data on-demand for extraction
...
enum PFS0Parser {
    static func parse(dataProvider: @escaping DataProvider, baseOffset: UInt64 = 0, magic: String = "PFS0", loadFileData: Bool = true) throws -> PFS0Partition {
        ...
        let readMagic: UInt32 = try readLE(from: headerData, at: 0)
        ...
        let fileCount: UInt32 = try readLE(from: headerData, at: 4)
        let stringTableSize: UInt32 = try readLE(from: headerData, at: 8)
        let reserved: UInt32 = try readLE(from: headerData, at: 12)
        ...
        // Read the metadata block (file entries + string table) in one go
        ...
        for i in 0..<Int(fileCount) {
            ...
            let offset: UInt64 = try readLE(from: metadata, at: currentEntryBaseOffset + 0)
            let size: UInt64 = try readLE(from: metadata, at: currentEntryBaseOffset + 8)
            let stringTableRelOffset: UInt32 = try readLE(from: metadata, at: currentEntryBaseOffset + 16)
            ...
            if isHFS0 {
                hashedSize = try readLE(from: metadata, at: currentEntryBaseOffset + 20)
                entryReserved = try readLE(from: metadata, at: currentEntryBaseOffset + 24)
